...
/* ============================================================
 *  DIRECT_INSTALL (直装免root) 模式说明
 *
 *  当定义了 DIRECT_INSTALL 宏时:
 *    - getPID() 直接返回 getpid(), 因为 .so 运行在游戏进程内
 *    - process_vm_readv / process_vm_writev 对自身进程不需要 root
 *    - get_module_base() 读取 /proc/self/maps 查找 libUE4.so 基址
 *    - 不需要 sharedUserId, 不需要 root, 不需要单独的悬浮窗 App
 *
 *  编译方法: ndk-build DIRECT_INSTALL=true
 * ============================================================ */
...
int m;
int pd;
unsigned long libbase, Gname, Uworld, wh, Arrayaddr, Matrix, oneself, Objaddr, ye, xu, Object, weapon,handheld, Mesh, human, Bone, Uleve, fov, xiazhui, BulletSpeed, shouchi,  wuqizz;
int Myteam;
int isBeta, nByte;
...
#ifdef DIRECT_INSTALL
	// 直装模式: .so 运行在游戏进程内, 直接返回自身 PID
	// process_vm_readv/writev 对自身进程不需要 root 权限
	return getpid();
#else
	int id = -1;
	DIR *dir;
...
struct Fevcror//写法不一样改的就不一样老自瞄是YX
{
    float Y;
    float X;
    float Roll;
